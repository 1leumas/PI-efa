...
export function generateTimetable(state: TimetableState) {
    ...
    for (const c of classrooms) {
        ...
        let startPeriod = 0;
        let endPeriod = classesPerDay;
        let totalPeriods = classesPerDay;
        let passes: { start: number, end: number }[] = [];

        if (c.shift === 'Tarde') {
            startPeriod = classesPerDay;
            endPeriod = classesPerDay * 2;
        ...
        for (const currentSubj of c.subjects) {
            ...
            const capableTeachers = teachers.filter(t => {
                const canTeachSubject = t.allowedSubjectIds.includes(currentSubj.subjectId);
                const hasAfternoonRequirement = c.shift !== 'Manhã' || currentSubj.weeklyClasses > (classesPerDay * daysPerWeek); // Rough estimate if we have overflow
                // We actually know the slot `p` inside the loop, so let's just make sure they teach the subject
                    ...
                    for (let d = 0; d < daysPerWeek && needed > 0; d++) {
                        ...
                        for (let p = pass.start; p < pass.end && needed > 0; p++) {
                            ...
                            if (!classroomSchedule[d][p]) {
                                let chosenTeacher = capableTeachers.find(t => {
